...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('phonecode'), 'has-success': fields.phonecode && fields.phonecode.valid }">
    <label for="phonecode" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.country.columns.phonecode') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.phonecode" v-validate="'required|integer'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('phonecode'), 'form-control-success': fields.phonecode && fields.phonecode.valid}" id="phonecode" name="phonecode" placeholder="{{ trans('admin.country.columns.phonecode') }}">
        <div v-if="errors.has('phonecode')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('phonecode') }}</div>
    </div>
</div>
